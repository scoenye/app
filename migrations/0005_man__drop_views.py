...
class Migration(SchemaMigration):
    ...
    def backwards(self, orm):
        db.execute("CREATE VIEW vw_first_assigned_dept AS \
                    SELECT result.item, result.department, \
        ...
        db.execute("CREATE VIEW vw_active_desktops AS \
                    ...
                    WHERE vw_last_assigned_dept.department = department.id) AND \
                          ...
                          hardware.item_type = item_type.id AND \
                          item_type.id = 21;")
        ...
        db.execute("CREATE VIEW vw_supported_hard_soft AS \
                    SELECT hardware.id, hardware.description, hardware.item_type, \
                           item_type.name AS item_type_name, hardware.hostname AS host_ver, \
                           vw_last_assigned_dept.department, vw_last_assigned_dept.location, \
                           vw_last_assigned_serial.serial_no \
                    FROM hardware, item_type, vw_last_assigned_serial, vw_last_assigned_dept \
                    WHERE (((hardware.id = vw_last_assigned_serial.item) AND \
                          (hardware.id = vw_last_assigned_dept.item)) AND \
                          (hardware.item_type = item_type.id)) UNION \
                    SELECT software.id, software.description, software.item_type, \
                    ...
                    WHERE (((software.id = vw_last_assigned_serial.item) AND \
                          (software.id = vw_last_assigned_dept.item)) AND \
                          (software.item_type = item_type.id));")
